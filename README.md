...
3. [Testing with HeidiSQL](#3-testing)
...
6. HeidiSQL
7. [Sample Dataset](http://www.mysqltutorial.org/mysql-sample-database.aspx)
...
```
Pada [file](https://github.com/abaar/distributed-database/tree/master/config/manager/config.ini) `config.ini` , terdapat beberapa konfigurasi yaitu :
```
...
[ndbd default]
NoOfReplicas=3
...
```
Adapun [file](https://github.com/abaar/distributed-database/tree/master/config/datanode/my.cnf) `my.cnf` hanya berisi alamat Manager yang telah kita buat tadi.
```
[mysql_cluster]
