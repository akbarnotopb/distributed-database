...
{{--           <div class="form-group">
            <div class="form-control--select">
              <select id="in-listing-name" class="form-control" name="type">
                <option value="sale" {{($SearchParam->type == "sale") ? "selected" : ""}}>Dijual</option>
                <option value="rent" {{($SearchParam->type == "rent") ? "selected" : ""}}>Disewa</option>
              </select>
            </div>
          </div> --}}
          <div class="form-group">
      ...
      <div class="listing__param" style="width: 100%">
        <a href="#" class="listing__param-link"><span class="listing__param-item param-items">{{ ($SearchParam->type=='sale')?'Dijual':'Disewa' }}</span></a>
          ...
          <a href="">
            <span class="listing__param-item param-items">
              Kecamatan: {{ $searchedsub }}
            </span>
          </a>
          @endif

        <a href="#" class="listing__param-link"><span class="listing__param-item param-items">  Rp.{{ $SearchParam->minprice }} - @if(strlen($SearchParam->maxprice)>=20) Harga Maksimal @else Rp.{{ $SearchParam->maxprice }} @endif</span></a>
      </div>
      ...
      <div class="listing__list" style="margin-top: 30px;">
        ...
        <div v-else-if="(properties.length == 0 && finish==true) || (filteredProperties.length==0)">
          ...
          <div class="listing__empty" style="margin-top: 20px">
...
Vue.component('select2', { //city
  ...
  var _vm=new Vue({
    ...
    methods: { 
      ...
      fetchData() {
        axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
        axios.get('{{ route('listings.property.search') }}?type={{$SearchParam->type}}&category={{ isset($SearchParam->category)?$SearchParam->category:'' }}&city={{isset($SearchParam->city)?$SearchParam->city:'' }}&subdistrict={{ isset($SearchParam->subdistrict)?$SearchParam->subdistrict:'' }}&minprice={{ $SearchParam->minprice }}&maxprice={{ $SearchParam->maxprice }}&ajax=1').then(response => {
      ...
      get(url){
        if(url!=null){
         ...
         this.loading=true;
         axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
         axios.get(url+'&type={{$SearchParam->type}}&category={{ isset($SearchParam->category)?$SearchParam->category:'' }}&city={{isset($SearchParam->city)?$SearchParam->city:'' }}&subdistrict={{ isset($SearchParam->subdistrict)?$SearchParam->subdistrict:'' }}&minprice={{ $SearchParam->minprice }}&maxprice={{ $SearchParam->maxprice }}&ajax=1').then(response=>{
          this.pagin=response.data;
      ...
      fetchCities(){
          axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
          axios.get('{{ Route('listings.property.search') }}?city=1&fetch=1&ajax=1').then(response => {
      ...
      fetchSubdistrict(city_id){
          axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
          axios.get('{{ Route('listings.property.search') }}?subdistrict=1&city_id='+city_id+'&fetch=1&ajax=1').then(response => {
