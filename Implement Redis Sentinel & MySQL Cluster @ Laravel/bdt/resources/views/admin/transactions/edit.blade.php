...
<div class="card">
  <div class="card-body">
    ...
    <div style="margin-bottom: 20px">
      ...
      <form class="form-horizontal" action="{{route('transactions.update',[$transaction->id])}}" method="post" style="{{session('success') ? 'display: none;' : ''}}" id="form-add-property" @submit="insertDesc">
        @method('PUT')
        @csrf
            <div class="form-group row">
                <div class="col-md-6 required">
                    ...
                    <select required name="agent_id" data-placeholder="---" class="form-control" @change="onChange()"  v-model="key">
                        <option value="0" {{($transaction->agent_id == 0) ? 'selected' : '' }}>ADMIN</option>
...
var quileditor;
  ...
  var form = new Vue({
    ...
    data: {
      propertySelected:"{{ (is_null(old('property_id'))?$transaction->property_id:old('property_id')) }}",
      properties_options:[],
      key:"{{ (is_null(old('agent_id'))?$transaction->agent_id:old('agent_id')) }}"
    },
    created(){
